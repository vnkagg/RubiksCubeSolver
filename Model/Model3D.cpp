...
class Model3D : public RubiksCube
{
private:
...
public:
    ...
    Model3D()
    {
        for (int i = 0; i < 6; i++)
        {
            for (int j = 0; j < 3; j++)
            {
                for (int k = 0; k < 3; k++)
                {
                    cube[i][j][k] = RubiksCube::getColorLetter(COLOR(i));
                }
            }
        }
    }

    COLOR getColor(RubiksCube::FACE face, unsigned row, unsigned col) const override
    {
    ...
    bool operator==(const Model3D &r1) const
    {
        for (int i = 0; i < 6; i++)
        {
            for (int j = 0; j < 3; j++)
    ...
    Model3D &operator=(const Model3D &r1)
    {
        for (int i = 0; i < 6; i++)
        {
            for (int j = 0; j < 3; j++)
...
struct Hash3d
{
    size_t operator()(const Model3D &r1) const
    {
        string str = "";
        for (int i = 0; i < 6; i++)
        {
